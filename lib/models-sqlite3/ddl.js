...
module.exports = `\
    ...
    CREATE TABLE tblVariableDef (id INTEGER PRIMARY KEY, unit TEXT Not Null, caption TEXT Not Null UNIQUE, code TEXT Not Null UNIQUE, ${fkey('User', true, 'provider')}, ${fkey('User', true, 'owner')}, ${fkey('User', true, 'reviewer')}, limit_lower DOUBLE, limit_upper DOUBLE, attribute TEXT);\
    ...
    CREATE TABLE tblReport (id INTEGER PRIMARY KEY, caption TEXT Not Null, title TEXT Not Null, time_limit INTEGER Not Null, ${fkey('User', true, 'owner')}, ${fkey('User', true, 'creator')}, ip_user TEXT Not Null, time_create INTEGER Not Null, time_reference INTEGER Not Null, pathfile TEXT, attribute TEXT);\
    CREATE TABLE tblVariable (id INTEGER PRIMARY KEY, unit TEXT Not Null, caption TEXT Not Null, code TEXT Not Null, ${fkey('User', true, 'provider')}, ${fkey('User', true, 'owner')}, ${fkey('User', true, 'reviewer')}, time_reference INTEGER Not Null, limit_lower DOUBLE, limit_upper DOUBLE, attribute TEXT);\
    ...
    CREATE VIEW tblvValue AS SELECT tblValue.value, MAX(tblValue.time_update) time_update, tblValue.variable_id, MAX(tblValue.id) id, tblValue.attribute, tblValue.ip_user value_ip_user, tblValue.base FROM tblValue GROUP BY tblValue.variable_id;\
    CREATE VIEW tblvVariable AS SELECT tblVariable.*, tblvValue.id value_id, tblvValue.value, tblvValue.time_update, tblvValue.attribute value_attribute, tblvValue.value_ip_user, tblvValue.base FROM tblVariable LEFT OUTER JOIN tblvValue ON tblVariable.id = tblvValue.variable_id;\
    CREATE VIEW tblvReportVariable AS SELECT tblReportVariable.*, tblvVariable.value_id, tblvVariable.value, tblvVariable.time_update, tblvVariable.value_attribute, tblvVariable.value_ip_user, tblvVariable.base FROM tblReportVariable LEFT OUTER JOIN tblvVariable ON tblvVariable.id = tblReportVariable.variable_id;\
    CREATE VIEW tblvReport AS SELECT tblReport.*, tblvReportVariable.weight, tblvReportVariable.variablecat_3_id, tblvReportVariable.id reportvariable_id, tblvReportVariable.value_id, tblvReportVariable.value, tblvReportVariable.time_update, tblvReportVariable.value_attribute, tblvReportVariable.value_ip_user, tblvReportVariable.base FROM tblReport LEFT OUTER JOIN tblvReportVariable ON tblvReportVariable.report_id = tblReport.id;\
    CREATE VIEW tblvTarget AS SELECT tblTarget.id, tblTarget.variable_id, tblTarget.value, tblTarget.time_target, tblTarget.time_update, tblTarget.user_update, tblTarget.ip_user, tblTarget.attribute FROM tblTarget;`;
