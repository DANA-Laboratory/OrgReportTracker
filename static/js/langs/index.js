const fa = {
  'addnew': 'جدید',
  'alert danger': 'اخطار',
  'alert default': 'عادی',
  'alert info': 'اطلاع',
  'alert name': 'پیام',
  'alert primary': 'جدی',
  'alert success': 'موفق',
  'alert warning': 'هشدار',
  'attach': 'پیوست',
  'attachments': 'پیوست ها',
  'attribute': 'مشخصات',
  'caption': 'عنوان',
  'categories': 'طبقه بندی شاخص ها',
  'cancel': 'انصراف',
  'cant delete': 'غیر قابل حذف',
  'close': 'بسته',
  'code': 'کد',
  'config': 'تنظیمات',
  'confirm': 'تایید',
  'confirm delete': 'حذف را تایید می نمایید؟',
  'confirm insert': 'ردیف جدید ثبت گردد؟',
  'confirm update': 'تغییرات ثبت گردد؟',
  'create new': 'جدید',
  'dashboard': 'داشبورد',
  'design': 'طراحی',
  'design & configurations': 'طراحی و تنظیم',
  'dont have an account!': 'حساب کابری ندارید!',
  'duration': 'دوره',
  'durations': ['1M', '3M', '6M', '12M'],
  'edit time': 'زمان ویرایش',
  'employee id code': 'کد پرسنلی',
  'error': 'خطا',
  'Error : element not in list!': 'خطا: گزینه در لیست وجود ندارد!',
  'department': 'اداره',
  'family': 'نام خانوادگی',
  'filter for categories': 'نمایش گروه های شاخص',
  'fname1': 'پرونده اکسل پیوست',
  'fname2': 'پرونده پیوست',
  'forgot password?': 'فراموش کرده اید؟',
  'from': 'از',
  'github account': 'کاربری در github',
  'github help': 'احراز هویت کاربر میتواند توسط پایگاه github انجام شود.',
  'go!': 'برو!',
  'inbox': 'صندوق دریافتی',
  'item removed': 'حذف شد',
  'item updated': 'تغییرات ثبت شد',
  'log out': 'خروج',
  'login': 'ورود',
  'lower limit': 'حد پایین',
  'name': 'نام',
  'need update': 'تغییرات را ثبت کنید',
  'new': 'جدید',
  'new attachment': 'پیوست دیگر',
  'new comments!': 'پیام جدید',
  'new item inserted': 'اطلاعات افزوده شد',
  'new orders!': 'درخواست جدید',
  'new tasks!': 'دستور جدید',
  'number of changes:': 'تعداد تغییرات',
  'open report': 'گزارش باز میباشد',
  'option 0': 'همه',
  'option 1': 'مالی',
  'option 2': 'ذی نفعان',
  'option 3': 'رشد و یادگیری',
  'owner': 'مالک',
  'password': 'کلمه عبور',
  'pi category': 'گروه',
  'pi entered': 'دستاورد های ثبت شده',
  'pi real': 'دستاورد',
  'pi registered': 'شاخص های گزارش',
  'pi title': 'عنوان شاخص',
  'pi weight': 'وزن',
  'pi': 'شاخص',
  'pis': 'شاخص ها',
  'provider': 'تامین کننده',
  'reader': 'دیدار',
  'remember me': 'بخاطر بسپار',
  'report': 'گزارش',
  'report progress': 'پیشرفت گزارش',
  'report title': 'عنوان گزارش',
  'reports archive': 'بایگانی گزارشات',
  'reports': 'گزارشات',
  'result 1': 'یک گزارش',
  'result 2': 'یک شاخص',
  'result 3': 'گزارش',
  'result 4': 'کاربر',
  'result 5': 'پیام',
  'remove': 'حذف',
  'report and cat_3': 'گزارش و زیر گروه',
  'reviewer': 'ناظر',
  'sample log': "how to add a line break here: <br/> must be on a new line.",
  'sample user': 'حامد روحانی',
  'sample variable': 'یک متغییر با نام نسبتا طولانی میتواند باشد',
  'select by type': 'بخشی از عبارت را وارد نمایید',
  'select duration': 'دوره را انتخاب کنید',
  'select owner': 'مالک را انتخاب کنید',
  'select provider': 'تامین کننده را انتخاب کنید',
  'select reviewer': 'ناظر را انتخاب کنید',
  'select workunit': 'اداره را انتخاب کنید',
  'settings': 'تنظیمات',
  'share': 'سهم(درصد)',
  'sign in': 'اشتراک',
  'sign up here': 'مشترک شوید',
  'related record exists': 'داده های مرتبط وجود دارد',
  'statistics overview': 'آمار کلی',
  'submit': 'ثبت',
  'summery of results': 'خلاصه نتایج',
  'support tickets!': 'گزارش جدید',
  'search': 'جستجو',
  'telegram account': 'نام کاربری در تلگرام',
  'telegram help': 'درصورت ثبت، احراز هویت کاربر میتواند از طریق telegram انجام شود.',
  'time left': 'زمان باقی مانده',
  'title': 'سیستم تحلیل گزارشات مدیریت',
  'unit': 'واحد',
  'upper limit': 'حد بالا',
  'user config': 'تنظیمات کاربران',
  'user log': 'فعالیت های کاربر:(کلیک نمایید)',
  'user progress': 'وضعیت کاربر',
  'user permission': 'مجوزهای کاربر',
  'username': 'نام کاربری',
  'users': 'کاربران',
  'view all': 'نمایش همه',
  'view details': 'بررسی ...',
  'variables': 'متغییرها',
  'weight': 'وزن',
  'workunit': 'اداره',
  'workunits': ['مالی', 'اداری', 'خدمات', 'HSE', 'آموزش', 'روابط عمومی', 'کنترل و نظارت'],
  'pi': 'سنجه',
  'pi real': 'دستاورد',
  'pi target': 'هدف',
  'pi category': 'گروه',
  'pi weight': 'وزن',
  'pi title': 'عنوان سنجه',
  'edit time': 'زمان ویرایش',
  'upper limit': 'حد بالا',
  'lower limit': 'حد پایین',
  'unit': 'واحد',
  'commands': ' ',
  'username help': 'این عبارت جهت ورود به سیستم استفاده خواهد شد.',
  'password help': 'کلمه عبور حداقل 5 حرف باشد.',
  'system administrator': 'مدیر سیستم',
  'ReportClass': {
    'header':  'گزارشات',
    'caption_cat_1': 'معادل برای مجموعه ها',
    'caption_cat_2': 'معادل برای گروهها',
    'caption_cat_3': 'معادل برای زیرگروهها',
    'caption_variable': 'معادل برای متغییرها',
  },
  'VariableCat': {
    'VariableCat_1':'مجموعه',
    'VariableCat_2':'گروه',
    'VariableCat_3':'زیر گروه',
  },
  'VariableCat_1': {
    'header':  'مجموعه ها',
  },
  'VariableCat_2': {
    'header': 'گروه ها',
  },
  'VariableCat_3': {
    'header': 'زیر گروه ها',
  },
  'VariableDef':{
    'header': 'متغییرها',
  },
  'vVariableDef': {
    'header': 'متغییرها',
  },
  'User': {
    'header': 'اطلاعات کاربری',
  },
};
if(typeof exports !== 'undefined') {
  module.exports = fa;
};
