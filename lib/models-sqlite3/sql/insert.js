...
exports.addReportClass = function (/*basedb*/ db, data) {
    return new Promise((resolve, reject) => {
        var p = (Number.isInteger(data.user_owner) ? Promise.resolve(data.user_owner) : sqlSelect.idfordata(db, {_tbl:'tblUser', account:data.user_owner}));
        p.then((owner_id)=>{
...
exports.addPIType_2 = function (/*basedb*/ db, data) {
  return new Promise((resolve, reject) => {
      var p = (Number.isInteger(data.pitype_1_id)) ? Promise.resolve(data.pitype_1_id) : sqlSelect.idfordata(db, {_tbl:'tblPIType_1', caption:data.pitype_1_id});
      p.then((pitype_1_id)=>{
...
exports.addPIType_3 = function (/*basedb*/ db, data) {
  return new Promise((resolve, reject) => {
      var p = (Number.isInteger(data.pitype_2_id)) ? Promise.resolve(data.pitype_2_id) : sqlSelect.idfordata(db, {_tbl:'tblPIType_2', caption:data.pitype_2_id});
      p.then((pitype_2_id)=>{
        db.pRun(`INSERT INTO ${data._tbl} ${data._clms} ${values(data._clmc)};`, [data.caption, data.code, pitype_2_id]).then(resolve).catch(reject);
      }).catch((err)=>{
        reject('addPIType_3() fails to fined related records with: ' + err);
      });
  });
};
...
exports.addReportVariable = function (/*basedb*/ db, data) {
  return new Promise((resolve, reject) => {
      var p1 = (Number.isInteger(data.variabledef_id)) ? Promise.resolve(data.variabledef_id) : sqlSelect.idfordata(db, {_tbl:'tblVariableDef', caption:data.variabledef_id});
      var p2 = (Number.isInteger(data.reportclass_id)) ? Promise.resolve(data.reportclass_id) : sqlSelect.idfordata(db, {_tbl:'tblReportClass', caption:data.reportclass_id});
      Promise.all([p1, p2]).then(([variabledef_id, reportclass_id])=>{
        ...
        reject('addReportVariable() fails to find related records with: ' + err);
      });
  });
};
