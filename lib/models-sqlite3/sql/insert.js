...
exports.addUser = function (/*basedb*/ db, data) {
    return require('bcrypt').hash(data.password, 10).then(function(hash) {
      db.pRun(`INSERT INTO ${data._tbl} ${data._clms} ${values(data._clmc)};`, [data.account, hash, data.fname, data.lname, data.pcode, data.workunit, data.sysadmin, data.attribute]);
    }).catch((err)=>{
...
exports.addReportClass = function (/*basedb*/ db, data) {
    return new Promise((resolve, reject) => {
        sqlSelect.idforaccount(db, data.user_owner).then((owner_id)=>{
          db.pRun(`INSERT INTO ${data._tbl} ${data._clms} ${values(data._clmc)};`, [data.caption, data.duration, owner_id, data.attribute]).then(resolve).catch(reject);
        }).catch((err)=>{
