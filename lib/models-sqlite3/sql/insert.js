...
exports.createReport = function (/*basedb*/ db, data) {
    return new Promise((resolve, reject) => {
        db.pRun(`INSERT INTO tblReport(caption, title, time_limit, user_owner, user_creator, ip_user, time_create) SELECT tblReportClass.caption, '${data.title}', ${data.time_limit}, tblReportClass.user_owner, ${data.user_creator}, '${data.ip_user}', ${data.time_create} FROM tblReportClass WHERE tblReportClass.id=${data.id}`)
        .then((newreport)=>{
            db.pRun(`INSERT INTO tblVariable(unit, caption, code, user_provider, user_owner, user_reviewer, limit_lower, limit_upper) SELECT tblVariableDef.unit, tblVariableDef.caption, tblVariableDef.code, tblVariableDef.user_provider, tblVariableDef.user_owner, tblVariableDef.user_reviewer, tblVariableDef.limit_lower, tblVariableDef.limit_upper FROM tblVariableDef INNER JOIN tblReportClassVariable ON tblReportClassVariable.variabledef_id = tblVariableDef.id WHERE tblReportClassVariable.reportclass_id = ${data.id}`)
            .then(()=>{
                db.pRun(`INSERT INTO tblReportVariable(report_id, variable_id, variablecat_3_id, weight) SELECT ${newreport.lastID} FROM tblReportClassVariable WHERE report_id=${data.id}`)
                .then(()=>{
                    resolve();
                });
            })
