...
router.post('/restful/:_tbl', function (req, res, next) {
  var data = req.body;
  //log an insert
  require('../logger').info('insert %s record_id=%s by account=%s from remote ip=%s', data.tostring(), req.user.account, req.connection.remoteAddress);
...
router.put('/restful/:_tbl/:_value', function (req, res, next) {
  var data = req.body;
  //log an update
  require('../logger').info('update %s record_id=%s by account=%s from remote ip=%s', data.tostring(), req.user.account, req.connection.remoteAddress);
  next("should not be here");
});
