...
function findOne(username, callback) {
    select.selectAll(db(), {_tbl: "tblUser", _where: "account", account: username}).then((users) => {
        if(users.length === 1){
            users[0].validPassword = (pass) => {
                return require('bcrypt-nodejs').compareSync(pass, users[0].password);
            };
